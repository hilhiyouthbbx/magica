...
// ─── Main Component (READ-ONLY public page) ─────────────────────────
    ...
    <div className="min-h-screen bg-[#080C14] text-white" style={{ fontFamily: "system-ui, sans-serif" }}>
      ...
      <div className="relative bg-gradient-to-b from-[#0D1520] to-[#080C14] px-4 pt-10 pb-8 text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          ...
          {/* Live day indicator — read-only */}
          {isLive && liveDay >= 0 ? (
            <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-sm bg-green-600 shadow-lg shadow-green-900/40">
              <span className="w-2 h-2 rounded-full bg-white animate-pulse" />
              Day {liveDay + 1} is Live!
            </div>
          ) : (
            <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-sm bg-white/10 text-white/40">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="rounded-2xl overflow-hidden border border-white/10 mb-6">
          ...
          {current.rows.map((row) => (
            <div
              key={row.id}
              className={`grid grid-cols-[90px_1fr_1fr] px-4 py-2.5 border-b border-white/5 last:border-0 items-start ${rowBg(row.type)}`}
            >
              {/* Time */}
